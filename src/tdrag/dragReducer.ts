...
export default class DragReducer {
  ...
  public pushElementsIDS(elementsIdList: string[]) {
    if (DragReducer.instance) {
      DragReducer.instance.elementIDS =
        DragReducer.instance.elementIDS.concat(elementsIdList);
    }
  }
  public pushElementID(elementId: string) {
    if (DragReducer.instance) {
      DragReducer.instance.elementIDS.push(elementId);
    }
  }
  /**
   * 生成递增标识
  ...
  public isChildren(ID: number) {
    if (DragReducer.instance) {
      DragReducer.instance.IDS.includes(ID);
    }
  }
  public elementIsChild(elementId: string) {
    if (DragReducer.instance) {
      DragReducer.instance.elementIDS.includes(elementId);
    }
  }
  /**
   * 同步集合与最大标识
  ...
  public createChild(HTMLElement: HTMLElement | null) {
    if (DragReducer.instance && HTMLElement) {
      DragReducer.instance.pushElementID(HTMLElement.id);
      const E = new DragElement(
        DragReducer.instance.autoGenerateElementId() ?? ERR_CHILD_ID
      );
      E.init(HTMLElement);
